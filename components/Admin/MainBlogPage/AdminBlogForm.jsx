...
const AdminBlogForm = ({userId}) => {
  ...
  return (
    <div >
   <h1 className="text-2xl text-center my-5 text-red-500 font-bold">Add New Blog</h1>
    <form action={formAction} className=" BlogForm flex flex-col gap-3 bg-slate-400 p-4 rounded-xl">
      ...
      <button className="bg-indigo-500 font-semibold active:scale-95 rounded-xl hover:bg-red-500 text-white active:text-black" >Add Blog</button>
      {state?.error}
    </form>
    </div>
  );
